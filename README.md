...
| M5Stack Tab5 / ESP32-P4 LC color | Experimental branch-only target. Builds a diagnostic Macintosh LC/color skeleton with 512KB ROM mmap validation, 68EC020/68020 Musashi scaffold, 4MB RAM/VRAM probes, indexed-color renderer scaffolding, Tab5 backlight/touch/display smoke diagnostics, disk trace scaffolding, and bounded ROM-entry probes. Latest probe reaches normal reset continuation, reset dispatch, NuBus/slot-video probe, RAM fill/check, and RAM lane/copy diagnostics, then selects the ROM serial monitor; no claimed LC boot yet. |
