...
static tab5_panel_kind_t active_panel_kind = TAB5_PANEL_UNKNOWN;
...
esp_err_t tab5_bsp_display_init(void) {
    ...
    ESP_LOGI(TAG, "initializing M5Stack BSP display path");
    ...
    active_panel_kind = detect_panel_kind();
    esp_err_t err;
    if (active_panel_kind == TAB5_PANEL_ST7123) {
        ESP_LOGI(TAG, "using ST7123 display path");
        err = bsp_display_new_with_handles_to_st7123(NULL, &display_handles);
    } else {
        ESP_LOGI(TAG, "using ILI9881C/ST7703-compatible display path (kind=%d)", active_panel_kind);
        err = bsp_display_new_with_handles(NULL, &display_handles);
    }
...
esp_err_t tab5_bsp_display_set_brightness(uint8_t percent) {
    ...
    ESP_RETURN_ON_ERROR(tab5_bsp_display_init(), TAG, "display init failed before brightness set");
...
esp_err_t tab5_bsp_display_flush_indexed(const uint8_t *indexed_pixels, size_t indexed_size,
    ...
    if (indexed_pixels == NULL || palette_rgb565 == NULL || indexed_size < LC_VIDEO_INDEXED_SIZE) {
        return ESP_ERR_INVALID_ARG;
    }
    ESP_RETURN_ON_ERROR(tab5_bsp_display_init(), TAG, "display init failed before indexed flush");

    ESP_LOGI(TAG,
             "flushing LC indexed framebuffer: guest=%ux%u scale=%u/%u physical=%ux%u offset=(%u,%u)",
             LC_VIDEO_WIDTH, LC_VIDEO_HEIGHT, TAB5_LC_VIEWPORT_SCALE_NUM,
             TAB5_LC_VIEWPORT_SCALE_DEN, TAB5_LC_VIEWPORT_W, TAB5_LC_VIEWPORT_H,
             TAB5_LC_OFFSET_X, TAB5_LC_OFFSET_Y);
    ...
    if (strip == NULL) {
        ESP_LOGE(TAG, "failed to allocate %u-row BSP flush strip", TAB5_STRIP_ROWS);
    ...
    for (int y = 0; y < BSP_LCD_V_RES; y += TAB5_STRIP_ROWS) {
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_panel_draw_bitmap(display_handles.panel, 0, y, BSP_LCD_H_RES,
                                                      y + rows, strip),
                            TAG, "draw indexed strip failed");
    }

    ESP_LOGI(TAG, "LC indexed framebuffer flushed to Tab5: physical_rgb565_checksum=0x%08" PRIx32,
             rgb565_checksum);
